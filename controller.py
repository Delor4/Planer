        ...
        for day in cal.itermonthdays2(self.date.year, self.date.month):
            if day[0] > 0:
                b = {'day_of_month': day[0],
                     # 'week_of_month': row,
                     'day_of_week': day[1],
                     'notes_count': self.db.count_textnotes(datetime.date(self.date.year, self.date.month, day[0])),
                     'images_count': self.db.count_images(datetime.date(self.date.year, self.date.month, day[0]))
                     }
                ret.append(b)
            # if day[1] == 6:
            #    row += 1
